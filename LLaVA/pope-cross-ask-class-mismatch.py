...
for item in [('dog', 'keyboard'), ('keyboard', 'dog')]:
    ...
    labels = [int(x[item_dict[item[1]]]==1) for x in labels] # ask one classify the other
    # print(labels)
    # exit()
    
    input_datas = torch.load(f'pope_info_probe_list{item[0]}.pth') # -{item[1]}
    print(len(labels), len(input_datas[0]))
    written_file=f"pope_result{item[0]}-{item[1]}-mismatch.txt"
