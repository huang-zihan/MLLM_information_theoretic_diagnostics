...
# 加载标签和输入数据
for item in ['chair', 'backpack']:
    
    vit=False
    if item in ['chair', 'backpack']:
        one_item=True
    else:
        one_item=False
    # 定义模型
    ...
    delect_list=item_to_image[item]
    indices_to_remove = []
    for index in delect_list:
        indices_to_remove.extend(range(index, index + 6))
    labels = [x for idx, x in enumerate(labels) if idx not in indices_to_remove]
    
    
    
    input_datas = torch.load(f'pope_info_probe_list{item}.pth')
    written_file=f"pope_result_{item}_counter.txt"
    
    for i, _ in enumerate(input_datas):
        input_datas[i]=[x for idx, x in enumerate(input_datas[i]) if idx not in indices_to_remove]
