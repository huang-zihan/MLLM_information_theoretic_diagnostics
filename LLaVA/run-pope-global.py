...
def eval_model(args):
    ...
    for i, item in enumerate(train_loader):
        ...
        print(i, "/", len(train_dataset), end="\r")
        # qs=item['question'][0] #"Please directly answer what is the number digit on the image by a digit."
        qs="Briefly answer what are in the image?"
        ###########
        # if label==1:
        #     if "Is there a " in qs:
        #         start = qs.find("Is there a ") + len("Is there a ")
    ...
    #     # break  # 只查看第一个批次
    torch.save(info_save_list, 'globalpope_info_probe_list.pth')
    # torch.save(label_list, 'globalpope_label_list.pth')
    torch.save(output_list, 'globalpope_output_list.pth')  # 新增保存模型输出的代码
    torch.save(question_list, 'globalpope_question_list.pth')  # 新增保存模型输出的代码
    # torch.save(vit_feature_list, 'globalpope_vit_feature_list.pth')  # 新增保存模型输出的代码
