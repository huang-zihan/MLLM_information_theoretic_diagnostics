...
def eval_model(args):
    ...
    for i, (image, label) in enumerate(train_dataset):
        ...
        qs="Please directly answer what is the number digit on the image by a digit."
        ...
        flattened_tensor = torch.flatten(image_tensor)
        ...
        #     break
        # break  # 只查看第一个批次
    torch.save(info_save_list, 'info_probe_list.pth')
    torch.save(label_list, 'label_list.pth')
    torch.save(output_list, 'output_list.pth')  # 新增保存模型输出的代码
    
    info_probe_list=torch.load('info_probe_list.pth')
    label_list=torch.load('label_list.pth')
    print(info_probe_list)
    print(label_list)
