...
def eval_model(args):
    ...
    dataset_type=args.dataset #"cifar10"
    mask=True
    
    if dataset_type=="cifar10":
        eval_dataset=train_dataset_cifar10
    elif dataset_type=="cifar100":
        eval_dataset=train_dataset_cifar100
    
    # 对CIFAR10数据集进行处理
    for i, (image, label) in enumerate(eval_dataset):
        print(i, "/", len(eval_dataset), end="\r")
        ...
        if mask==True:
            ...
            # 设定蒙版的上下边界和左右边界
            upper_bound = image_np.shape[0] // 4  # 上边界为高度的四分之一
            lower_bound = 3 * image_np.shape[0] // 4  # 下边界为高度的四分之三
            left_bound = image_np.shape[1] // 4  # 左边界为宽度的四分之一
            right_bound = 3 * image_np.shape[1] // 4  # 右边界为宽度的三分之三

            # 设定黑色蒙版区域
            image_np[upper_bound:lower_bound, left_bound:right_bound] = 0
    ...
    if dataset_type=="cifar10":
        # 保存信息探测列表和标签列表
        if mask==True:
            torch.save(info_save_list, 'info_probe_list_cifar10_mask.pth')
            torch.save(label_list, 'label_list_cifar10_mask.pth')
            torch.save(output_list, 'cifar10_output_list_mask.pth')  # 新增保存模型输出的代码
        else:
            torch.save(info_save_list, 'info_probe_list_cifar10.pth')
            torch.save(label_list, 'label_list_cifar10.pth')
            torch.save(output_list, 'cifar10_output_list.pth')  # 新增保存模型输出的代码
    elif dataset_type=="cifar100":
        # 保存信息探测列表和标签列表
        if mask==True:
            torch.save(info_save_list, 'info_probe_list_cifar100_mask.pth')
            torch.save(label_list, 'label_list_cifar100_mask.pth')
            torch.save(output_list, 'cifar100_output_list_mask.pth')  # 新增保存模型输出的代码
        else:
