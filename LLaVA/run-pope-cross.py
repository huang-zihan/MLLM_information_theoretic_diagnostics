...
def eval_model(args):
    ...
    #'dining table''car'# args.question
    indices_to_remove = []
    ...
    train_dataset = [item for idx, item in enumerate(train_dataset) if idx in indices_to_remove]
    ...
    for i, item in enumerate(train_loader):
        ...
        if args.cross_question=="":
            qs=item['question'][0] #"Please directly answer what is the number digit on the image by a digit."
        else:
            if args.cross_question[0] in ['a', 'e', 'i', 'o', 'u']:
                qs="Is there an " +args.cross_question+ " in the image?"
            else:
                qs="Is there a " +args.cross_question+ " in the image?"
        ###########
        # if label==1:
        #     if "Is there a " in qs:
    ...
    if args.question=="":
        ...
        torch.save(info_save_list, f'pope_info_probe_list{args.question}-{args.cross_question}.pth')
        # torch.save(label_list, 'pope_label_list.pth')
        torch.save(output_list, f'pope_output_list{args.question}-{args.cross_question}.pth')  # 新增保存模型输出的代码
        torch.save(question_list, f'pope_question_list{args.question}-{args.cross_question}.pth')  # 新增保存模型输出的代码
        torch.save(vit_feature_list, f'pope_vit_feature_list{args.question}-{args.cross_question}.pth')  # 新增保存模型输出的代码
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cross_question", type=str, default="")
