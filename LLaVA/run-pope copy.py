...
def eval_model(args):
    ...
    # {'item_to_image': item_to_image, 'image_to_item': image_to_item}
    # item_to_image=dict()
    # image_to_item=dict()
    # index=0
    ...
    #             # 如果物品不在字典中，则添加
    #             if object_name not in item_to_image:
    #                 item_to_image[object_name] = set([index])
    #             else:
    #                 item_to_image[object_name].add(index)
                
    #             if index not in image_to_item:
    #                 image_to_item[index]=set([item_dict[object_name]])
    #             else:
    #                 image_to_item[index].add(item_dict[object_name])
        
    #     if (i+1)%6==0:
    #        index+=1
    # print(item_to_image)
    # print(image_to_item)
    # torch.save({'item_to_image': item_to_image, 'image_to_item': image_to_item}, 'pope_meta_data.pth')
    # exit()
    # item_dict = {}
    # cnt_dict = {}
    # index = 0
    ...
    #     # 提取物品（假设物品在问题中以"Is there a"和"in the image?"之间）
    #     # 提取物品名称
    #     if "Is there a " in qs:
    #         start = qs.find("Is there a ") + len("Is there a ")
    #     elif "Is there an " in qs:
    ...
    #         # # 如果物品不在字典中，则添加
    #         # if object_name not in item_dict:
    #         #     item_dict[object_name] = index
    #         #     index += 1
            
    #         # 如果物品不在字典中，则添加
    #         if object_name not in cnt_dict:
    #             cnt_dict[object_name] = 1
    #         else:
    #             cnt_dict[object_name] += 1
    # print(cnt_dict)
    ...
    for i, item in enumerate(train_loader):
        ...
        print("############") # torch.Size([1, 48]) 1317 -> 29871,  -200, 29871,    13,  3624,
        #######################
        ...
        with torch.inference_mode():
            ...
            output_ids = model.generate(
                ...
                max_new_tokens=1024,
                use_cache=True)
            # print("output len", len(output_ids[0]))
            # print('done')
            
            print(model.info_probe_list[0])
            print(model.image_info_probe_list)
            exit()
