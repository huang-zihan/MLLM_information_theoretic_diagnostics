...
# F.kl_div(torch.log_softmax(predicted[j], dim=0), target[j].float())
...
image_only=False
# classify_types=["pope", "vqa", "chair", "hal"] # , 
# classify_types=["mnist"]
classify_types=["coco_caption", "vqa", "aokvqa", "hal"]

for _, classify_type in enumerate(classify_types):
    if idx!=-1:
        classify_type=classify_types[idx]

    print("process:", classify_type)
    ...
    class SimpleClassifier(nn.Module):
        def __init__(self):
            ...
            self.fc3 = nn.Linear(128, 80)  # 10个分类
    ...
    num_epochs = 400
    prefix="origin"
    # if classify_type=="pope":
    #     labels = torch.load(f'../cbm/pope/{prefix}pope_label_list_v1.pth')
    #     input_datas = torch.load(f'../cbm/pope/{prefix}{"image_" if image_only else ""}pope_info_probe_list.pth')
    if classify_type=="vqa":
        labels = torch.load(f'../cbm/vqa/{prefix}vqa_ce_val_label_full_v1.pth')
        input_datas = torch.load(f'../cbm/vqa/{prefix}vqa_ce_val_{"image_" if image_only else ""}full_v1.pth')
        print("loaded from ", f'../cbm/vqa/{prefix}vqa_ce_val_{"image_" if image_only else ""}full_v1.pth')
    # elif classify_type=="chair":
    #     labels = torch.load('../cbm/chair/chair_label_list_v1.pth')
    #     input_datas = torch.load(f'../cbm/chair/{"image_" if image_only else ""}chair_info_probe_list_v1.pth')
    elif classify_type=="hal":
        labels = torch.load(f'../cbm/hal/{prefix}wrong_hal_ce_val_label_full_v1.pth')
        input_datas = torch.load(f'../cbm/hal/{prefix}wrong_hal_ce_val_{"image_" if image_only else ""}full_v1.pth')
        print("loaded from ", f'../cbm/hal/{prefix}wrong_hal_ce_val_{"image_" if image_only else ""}full_v1.pth')
    elif classify_type=="aokvqa":
        labels = torch.load(f'../cbm/aokvqa/{prefix}wrong_aokvqa_label_list_v1.pth')
        input_datas = torch.load(f'../cbm/aokvqa/{prefix}wrong_{"image_" if image_only else ""}aokvqa_info_probe_list_v1.pth')
        print("loaded from ", f'../cbm/aokvqa/{prefix}wrong_{"image_" if image_only else ""}aokvqa_info_probe_list_v1.pth')
    elif classify_type=="coco_caption":
        labels = torch.load(f'../cbm/coco_caption/{prefix}coco_caption_ce_val_label_full_v1.pth')
        input_datas = torch.load(f'../cbm/coco_caption/{prefix}coco_caption_ce_val_{"image_" if image_only else ""}full_v1.pth')
        print("loaded from ", f'../cbm/coco_caption/{prefix}coco_caption_ce_val_{"image_" if image_only else ""}full_v1.pth')
    ...
    missing_index=[
        ...
        [68, 69, 70, 270, 271, 272, 273, 928, 929, 930, 1240, 1241, 1242, 1502, 1503, 1504, 3823, 3824, 3825, 3826, 3827, 3828, 3829, 3830, 3831, 3912, 3913, 3914, 3915, 3920, 3921, 3922, 3923, 3924, 4171, 4172, 4173, 4174, 4175, 4176, 4177, 5159, 5160, 5161, 5162, 5163, 5164, 5165, 5166, 5167, 5168, 5169, 5170, 5171, 5172, 5173, 5174, 5175, 5176, 5177, 5178, 5179, 5180, 5181, 5182, 5183, 5944, 5945, 5946, 5947, 6007, 6008, 6009, 6803, 6804, 6805, 6806, 6807, 6808, 6809, 6810, 6811, 6812, 7022, 7023, 7024, 7025, 7026, 7027, 7028, 7029, 7030, 7031, 7032, 7033, 7034, 7229, 7230, 7231, 7609, 7610, 7611, 7771, 7772, 7773, 7774, 8054, 8055, 8056, 8270, 8271, 8272, 8505, 8506, 8507, 8508, 8509, 8931, 8932, 8933, 8934, 8935, 9600, 9601, 9602, 9603, 9604, 9605, 9606, 9607, 9608, 9609, 9863, 9864, 9865, 9866, 9867, 9868, 9869, 9870, 9871, 9872, 9873, 9992, 9993, 9994, 9995, 9996, 9997, 9998, 9999],
        [2385, 2386, 2387, 2625, 2626, 2627, 2928, 2929, 2930, 3222, 3223, 3224], # wrong
        [128, 129, 130, 131, 136, 137, 138, 139, 520, 521, 522, 523, 1064, 1065, 1066, 1067, 1896, 1897, 1898, 1899, 2068, 2069, 2070, 2071, 2340, 2341, 2342, 2343, 2620, 2621, 2622, 2623, 2812, 2813, 2814, 2815, 3384, 3385, 3386, 3387, 3972, 3973, 3974, 3975, 4292, 4293, 4294, 4295, 4496, 4497, 4498, 4499, 5384, 5385, 5386, 5387, 5552, 5553, 5554, 5555, 6044, 6045, 6046, 6047, 6188, 6189, 6190, 6191, 6356, 6357, 6358, 6359, 6488, 6489, 6490, 6491, 6964, 6965, 6966, 6967, 7496, 7497, 7498, 7499, 7540, 7541, 7542, 7543, 7656, 7657, 7658, 7659, 7956, 7957, 7958, 7959, 8076, 8077, 8078, 8079, 8084, 8085, 8086, 8087, 8096, 8097, 8098, 8099, 8604, 8605, 8606, 8607, 8700, 8701, 8702, 8703, 9040, 9041, 9042, 9043, 9060, 9061, 9062, 9063, 9612, 9613, 9614, 9615, 9628, 9629, 9630, 9631, 10112, 10113, 10114, 10115, 10292, 10293, 10294, 10295, 10488, 10489, 10490, 10491, 11192, 11193, 11194, 11195, 11212, 11213, 11214, 11215, 11800, 11801, 11802, 11803, 12036, 12037, 12038, 12039, 12760, 12761, 12762, 12763, 13324, 13325, 13326, 13327, 13720, 13721, 13722, 13723, 13856, 13857, 13858, 13859, 14152, 14153, 14154, 14155, 14260, 14261, 14262, 14263, 14280, 14281, 14282, 14283, 14348, 14349, 14350, 14351, 14356, 14357, 14358, 14359, 14364, 14365, 14366, 14367, 14408, 14409, 14410, 14411, 14420, 14421, 14422, 14423, 14908, 14909, 14910, 14911, 14916, 14917, 14918, 14919, 14928, 14929, 14930, 14931, 15204, 15205, 15206, 15207, 15344, 15345, 15346, 15347, 15360, 15361, 15362, 15363, 15372, 15373, 15374, 15375, 15784, 15785, 15786, 15787, 15860, 15861, 15862, 15863, 16300, 16301, 16302, 16303, 16468, 16469, 16470, 16471, 16972, 16973, 16974, 16975, 17400, 17401, 17402, 17403, 17412, 17413, 17414, 17415, 17424, 17425, 17426, 17427, 17696, 17697, 17698, 17699, 17816, 17817, 17818, 17819, 17820, 17821, 17822, 17823, 17936, 17937, 17938, 17939, 18412, 18413, 18414, 18415, 18448, 18449, 18450, 18451, 18840, 18841, 18842, 18843, 18924, 18925, 18926, 18927, 19408, 19409, 19410, 19411, 19416, 19417, 19418, 19419] # wrong
    ]
    
    labels=[x for k, x in enumerate(labels) if k not in missing_index[idx]]
    
    for i, index in enumerate(info_save_index):
        
        input_data=input_datas[i]
        input_data=[x for k, x in enumerate(input_data) if k not in missing_index[idx]]
        print(f"-----------------index {index}-----------------")
        # 初始化模型、损失函数和优化器
        ...
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                for inputs, target in test_loader:
                    ...
                    kl_list+=[F.kl_div(torch.log_softmax(predicted[j], dim=0), target[j].float()) for j in range(len(predicted))]
            kl=sum(kl_list)/len(kl_list)
            print(f'Epoch [{epoch+1}/{num_epochs}], Loss: {loss.item():.4f}, KL: {kl:.6f}')
    ...
    if classify_type=="pope":
        written_file=f'result/{"image_" if image_only else ""}pope_result.txt'
    elif classify_type=="vqa":
        written_file=f'result/{"image_" if image_only else ""}vqa_result.txt'
    elif classify_type=="chair":
        written_file=f'result/{"image_" if image_only else ""}chair_result.txt'
    elif classify_type=="hal":
        written_file=f'result/{"image_" if image_only else ""}hal_result.txt'
    elif classify_type=="aokvqa":
        written_file=f'result/{"image_" if image_only else ""}aokvqa_result.txt'
    elif classify_type=="coco_caption":
        written_file=f'result/{"image_" if image_only else ""}coco_caption_result.txt'
    
    with open(written_file, "w") as f:
        f.write("Index\tEpoch\tLoss\tAccuracy\n")
        for index, metrics in results.items():
            for epoch in range(num_epochs):
                f.write(f"{index}\t{epoch + 1}\t{metrics['losses'][epoch]:.4f}\t{metrics['accuracies'][epoch]}\n")
