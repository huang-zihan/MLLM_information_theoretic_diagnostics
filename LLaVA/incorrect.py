...
labels = torch.load('label_list_cifar100_incorrect.pth')
input_datas = torch.load('info_probe_list_cifar100_incorrect.pth')

test_labels = torch.load('label_list_cifar100_incorrect_test.pth')
test_input_datas = torch.load('info_probe_list_cifar100_incorrect_test.pth')

print(len(test_labels), len(test_input_datas), len(labels), len(input_datas))
...
for i, index in enumerate(info_save_index):
    ...
    test_input_data=test_input_datas[i]
    ...
    # 将数据转换为 Tensor
    test_data_tensor = torch.stack(test_input_data)  # (10, 1024) 假设每个 tensor 是 1024 维
    test_labels_tensor = torch.tensor(test_labels, device='cuda:0')

    # 创建 TensorDataset
    dataset = TensorDataset(data_tensor, labels_tensor)
    testdataset=TensorDataset(test_data_tensor, test_labels_tensor)
