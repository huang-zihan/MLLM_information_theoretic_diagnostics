...
def eval_model(args):
    ...
    annotations_file = '../cbm/coco_caption/coco_caption_ce_data_full_v1.pth'  
    ...
    for i, item in enumerate(train_loader):
        ...
        if "origin" in args.question:
            qs=item['response'][0][:-1] + " on the image?\n"
        elif "fixed_question" not in args.question:
        ...
        for j, index in enumerate(info_save_index):
            # print(len(hidden_states[index][0][image_tokens_range[-1][-1]+1:].cpu()), image_tokens_range[-1][-1]+1)
            # exit()
    ...
    if '_full' in annotations_file:
        torch.save(info_save_list, ce_data_dir+f'{args.question}coco_caption_ce_val_full_v1.pth')
        torch.save(avg_info_save_list, ce_data_dir+f'{args.question}avg_coco_caption_ce_val_full_v1.pth')
        torch.save(image_info_save_list, ce_data_dir+f'{args.question}coco_caption_ce_val_image_full_v1.pth')
        torch.save(label_list, ce_data_dir+f'{args.question}coco_caption_ce_val_label_full_v1.pth')
        torch.save(question_list, ce_data_dir+f'{args.question}coco_caption_ce_val_question_full_v1.pth')
    else:
        torch.save(info_save_list, ce_data_dir+f'{args.question}coco_caption_ce_val_v1.pth')
        orch.save(avg_info_save_list, ce_data_dir+f'{args.question}avg_coco_caption_ce_val_v1.pth')
        torch.save(image_info_save_list, ce_data_dir+f'{args.question}coco_caption_ce_val_image_v1.pth')
        torch.save(label_list, ce_data_dir+f'{args.question}coco_caption_ce_val_label_v1.pth')
        torch.save(question_list, ce_data_dir+f'{args.question}coco_caption_ce_val_question_v1.pth')
