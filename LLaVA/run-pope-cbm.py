...
class POPEDataset(Dataset):
    def __init__(self, dataset, image_processor, model, visual_prompt=False, cbm_path='./',):
    ...
    def __getitem__(self, idx):
        ...
        label = self.annotation[idx]['annotation']  # 确保这里的'key'与你的数据集一致
...
def eval_model(args):
    ...
    pope_dataset = POPEDataset(train_dataset, image_processor, model, visual_prompt)
    ...
    if args.question=="":
        ...
        torch.save(image_info_save_list, cbm_path+'image_pope_info_probe_list.pth')
        ...
        torch.save(info_save_list, cbm_path+'vp_pope_info_probe_list.pth')
        torch.save(image_info_save_list, cbm_path+'vp_image_pope_info_probe_list.pth')
    else:
        torch.save(info_save_list, cbm_path+f'pope_info_probe_list{args.question}.pth')
        torch.save(image_info_save_list, cbm_path+f'image_pope_info_probe_list{args.question}.pth')
