...
# print(labels[0])
# exit()
# question_list=["irrelevant", "visual_prompt"] #["person", "bus", "car", "airplane", "bird", "dog", "elephant", "fork", "cell phone", "tv"]
# question_list=["python code", "email", "aircraft", "operating system", "alien"]
# question_list=["irrelevant-2", "irrelevant-3", "irrelevant-4", "irrelevant-5"]
# question_list=["visual_prompt", "visual_prompt-1", "visual_prompt-2"]
# question_list=["indom", "outdom", "insuff_att", "amb_desc"]
question_list=[""]
text_margin=False
...
for question in question_list:
    ...
    if question!="":
        input_datas = torch.load(ce_data_dir+f'pope_info_probe_list{question}_v1.pth')
    else:
        if image_only:
            input_datas = torch.load(ce_data_dir+'image_pope_info_probe_list.pth')
        else:
            input_datas = torch.load(ce_data_dir+'pope_info_probe_list.pth')
        
    labels = torch.load(ce_data_dir+'pope_label_list_v1.pth')
    ...
    for margin_num in margin_num_list:
        if text_margin:
            if question!="":
                written_file=f'result/all_kls_text_margin_{margin_num}_trainepoch{epoch}_{question}_v1'
                pic_file=f'result/line_chart_text_margin_{margin_num}_trainepoch{epoch}_{question}_v1'
            else:
                if image_only:
                    written_file=f'result/image_all_kls_text_margin_{margin_num}_trainepoch{epoch}_v1'
                    pic_file=f'result/image_line_chart_text_margin_{margin_num}_trainepoch{epoch}_v1'
                else:
                    written_file=f'result/all_kls_text_margin_{margin_num}_trainepoch{epoch}_v1'
                    pic_file=f'result/line_chart_text_margin_{margin_num}_trainepoch{epoch}_v1'
        else:
            if question!="":
                written_file=f'result/all_kls_merge_trainepoch{epoch}_{question}_v1'
                pic_file=f'result/line_chart_merge_trainepoch{epoch}_{question}_v1'
            else:
                if image_only:
                    written_file=f'result/image_all_kls_merge_trainepoch{epoch}_v1'
                    pic_file=f'result/image_line_chart_merge_trainepoch{epoch}_v1'
                else:
                    written_file=f'result/all_kls_merge_trainepoch{epoch}_v1'
                    pic_file=f'result/line_chart_merge_trainepoch{epoch}_v1'
            
            if seperate_layer:
                if question!="":
                    pic_file=f'result/line_chart_seperatelayer_trainepoch{epoch}_{question}_v1'
                else:
                    pic_file=f'result/line_chart_seperatelayer_trainepoch{epoch}_v1'
