...
for margin_num in margin_num_list:
    if text_margin:
        written_file=f'result/all_kls_text_margin_{margin_num}_trainepoch{epoch}.txt'
        pic_file=f'result/line_chart_text_margin_{margin_num}_trainepoch{epoch}.pdf'
    else:
        written_file=f'result/all_kls_merge_trainepoch{epoch}.txt'
        pic_file=f'result/line_chart_merge_trainepoch{epoch}.pdf'
        if seperate_layer:
            pic_file=f'result/line_chart_seperatelayer_trainepoch{epoch}.pdf'
    ...
    if seperate_layer == False:
        model = CE()
        model.load_state_dict(torch.load(f'./result/ckpt/ce_model_merge_epoch{epoch}.pth'))
        model.to('cuda:0')
    ...
    for i, index in enumerate(info_save_index):
        ...
        if seperate_layer:
            ...
            #     break
            model = CE()
            model.load_state_dict((torch.load(f'./result/ckpt/ce_model_{i}_epoch{epoch}.pth')))
