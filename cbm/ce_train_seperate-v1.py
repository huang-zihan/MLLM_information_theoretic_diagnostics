...
for i in range(len(info_save_index)):    
    if i==0:
        ...
        input_datas[i]=[input_datas[i][j] for j in range(len(input_datas[i])) if j not in coco_missing_obj_list ]
    # print("input data of i length", len(input_datas[i]))
####
...
for i, index in enumerate(info_save_index):
    ...
    print("shape of data_tensor, labels_tensor ", labels_tensor.shape, data_tensor.shape)
    # 创建 TensorDataset
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch+1)%3==0:
            torch.save(model.state_dict(), f'./result/ckpt/ce_model_{index}_epoch{epoch}_v1.pth')
    ...
    torch.save(model.state_dict(), f'./result/ce_model_{index}_v1.pth')
